...
// session (not just page load) turns effects off immediately.
...
function MatrixRain({
  ...
  }, [fontSize, color, trail, speed, reducedMotion]);
  return <canvas ref={ref} className={className} style={{ opacity, display: "block", width: "100%", height: "100%", ...styleProp }} />;
}

function NoiseOverlay() { const ref = useRef<HTMLCanvasElement>(null); const reducedMotion = usePrefersReducedMotion(); useEffect(() => { const canvas = ref.current; if (!canvas || reducedMotion) return; const SIZE = 120; canvas.width = SIZE; canvas.height = SIZE; const ctx = canvas.getContext("2d")!; let raf: number; function draw() { const d = ctx.createImageData(SIZE, SIZE); for (let i = 0; i < d.data.length; i += 4) { const v = Math.random() > 0.93 ? Math.floor(Math.random() * 200) : 0; d.data[i] = 0; d.data[i + 1] = v; d.data[i + 2] = 0; d.data[i + 3] = v; } ctx.putImageData(d, 0, 0); raf = requestAnimationFrame(draw); } raf = requestAnimationFrame(draw); return () => cancelAnimationFrame(raf); }, [reducedMotion]); return <canvas ref={ref} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", opacity: 0.04, pointerEvents: "none", imageRendering: "pixelated" }} />; }
...
// section headers (00 AIConsierge – 04 Contact) stay pixel-identical in
// font, size and color by construction — a local copy per component was how
// they drifted apart (AIConsierge/Contact ended up on mono+green, Contact
// also ran a much bigger clamp) before this was unified.
const SECTION_TITLE_STYLE: React.CSSProperties = { fontFamily: "'Montserrat',sans-serif", fontWeight: 700, fontSize: "clamp(20px,4.4vh,40px)", color: "#f2f2f2", marginTop: 14 };
// Checked fresh at call time rather than via a hook — these are plain
// functions invoked from event handlers, not component render, and a
// synchronous matchMedia read is cheap enough to not need a subscription.
...
function goToSection(id: string) {
  ...
  document.getElementById(id)?.scrollIntoView({ behavior: scrollBehaviorPref(), inline: "start", block: "nearest" });
}
...
function getMainSections(): HTMLElement[] {
  const main = document.getElementById("top");
  if (!main) return [];
  return (Array.from(main.children) as HTMLElement[]).filter(el => el.dataset.navSkip !== "true");
}
...
function goToSectionIndex(index: number) {
  ...
  sections[clamped]?.scrollIntoView({ behavior: scrollBehaviorPref(), inline: "start", block: "nearest" });
}
...
const STATUS_PHRASES=["Пишу код сам.","Без шаблонов.","Без посредников.","На связи всегда."];
...
function useWordFlicker(text: string, active: boolean, initialDelay: number) {
  ...
  useEffect(() => {
    if (!active || reducedMotion) { setDisplay(text); return; }
    let cancelled = false;
    let toId = 0, ivId = 0;
  ...
  }, [active, text, initialDelay, reducedMotion]);
...
const MISSION_HEADLINE = [["Мне", "важно,", "чтобы"], ["сайт", "приносил", "заявки."]];
const MISSION_LINES = [
  { words: ["Не", "просто", "верстаю", "макет,"], pad: "0" },
  { words: ["а", "думаю", "о", "воронке."], pad: "clamp(28px,5vw,80px)" },
  { words: ["Если", "вижу", "слабое", "место,"], pad: "clamp(14px,2.5vw,40px)" },
  { words: ["говорю", "и", "сразу", "чиню."], pad: "clamp(42px,7vw,110px)" },
];
function Mission(){const ref=useRef<HTMLDivElement>(null);const[active,setActive]=useState(false);useEffect(()=>{const el=ref.current;if(!el)return;const ob=new IntersectionObserver(([e])=>{setActive(e.isIntersecting)},{threshold:.15});ob.observe(el);return()=>ob.disconnect()},[]);
  ...
  return <section id="mission" style={{flexShrink:0,scrollSnapAlign:"start",width:"100vw",height:"100vh",display:"flex",flexDirection:"column",justifyContent:"center",padding:"84px clamp(20px,5vw,90px) 20px",background:"#000",position:"relative",overflow:"hidden"}}><div style={{position:"absolute",inset:0}}><MatrixRain opacity={.26} fontSize={14} color="#00ff41" trail="rgba(0,0,0,.04)" speed={90}/></div><div style={{position:"absolute",left:"clamp(4px,1.4vw,16px)",top:"50%",width:0,height:0,pointerEvents:"none"}}><div style={{position:"absolute",top:0,left:0,width:340,height:20,overflow:"hidden",transform:"rotate(-90deg)",transformOrigin:"top left"}}><div style={{display:"inline-flex",gap:28,animation:"marqAnim 22s linear infinite",fontFamily:"'JetBrains Mono',monospace",fontWeight:700,fontSize:13,letterSpacing:".28em",textTransform:"uppercase",color:"rgba(0,255,65,.55)",textShadow:"0 0 12px rgba(0,255,65,.35)",whiteSpace:"nowrap"}}><span>МИССИЯ · MISSION · МИССИЯ · </span><span>МИССИЯ · MISSION · МИССИЯ · </span></div></div></div><div style={{position:"relative",zIndex:1,paddingLeft:"clamp(46px,7vw,96px)"}}>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:"clamp(4px,.8vh,10px)"}}>
      {MISSION_LINES.map((l, i) => { const bold = i % 2 === 0; return <div key={i} style={{paddingLeft:l.pad}}><span style={{fontFamily:"'JetBrains Mono',monospace",fontWeight:bold?700:300,fontSize:"clamp(15px,3vh,30px)",letterSpacing:"-.01em",display:"inline-block"}}>
        {l.words.map((w, wi) => {
          if (i === 0 && w === "макет,") return <span key={wi}><Glitch>макет,</Glitch>{wi < l.words.length - 1 ? " " : ""}</span>;
  ...
  </div><MobileScrollHint sectionId="mission"/></section>;}
function SectionRain({ opacity = 0.3, speed = 85, color = "#00ff41", className }: { opacity?: number; speed?: number; color?: string; className?: string }) {
  return <div className={className} style={{ position: "absolute", inset: 0, zIndex: 0, pointerEvents: "none" }}><MatrixRain opacity={opacity} fontSize={13} color={color} trail="rgba(0,0,0,.05)" speed={speed} /></div>;
}
...
function ProcessBar({ target, delay, animate, scanSpeed }: { target: number; delay: number; animate: boolean; scanSpeed: number }) {
  ...
  return (
    <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 13, letterSpacing: "-.05em", whiteSpace: "nowrap", display: "flex", alignItems: "center", gap: 10 }}>
      ...
      <span style={{ fontSize: 10, color: "rgba(0,255,65,.58)" }}>{fill}%</span>
    </div>
  );
}
function AIConsierge() {
  ...
  return (
    <section id="ai" style={{ flexShrink: 0, scrollSnapAlign: "start", width: "max(1100px,100vw)", height: "100vh", display: "flex", flexDirection: "column", justifyContent: "center", padding: "84px clamp(20px,5vw,60px) 20px", background: "#050f05", position: "relative", overflow: "hidden" }}>
      ...
      <div ref={ref} style={{ position: "relative", zIndex: 1 }}>
        <Reveal>
          <div className="section-header" style={{ marginBottom: 24 }}>
            <AppWindow skin={APP_SKINS.claude}>
              ...
              <h2 className="section-header-title" style={SECTION_TITLE_STYLE}>
                Агентский результат. Без агентского прайса.
              </h2>
            </AppWindow>
          </div>
        </Reveal>
        <Reveal delay={100}>
          <div style={{ border: "1px solid rgba(0,255,65,.2)", animation: "borderGlow 4s ease-in-out infinite" }}>
            ...
            <div className="ai-table-head" style={{ display: "grid", gridTemplateColumns: "44px 1fr 300px", gap: "0 clamp(12px,2vw,28px)", padding: "8px 16px 6px", borderBottom: "1px solid rgba(0,255,65,.1)", ...mono, fontSize: 10, letterSpacing: ".14em", textTransform: "uppercase", color: "rgba(0,255,65,.58)" }}>
              <span>PID</span><span>СЕРВИС</span><span>ПРОГРЕСС / СТАТУС</span>
            </div>
            {procs.map((p, i) => {
              const glow = rowGlow(p.status);
              return (
                <div key={p.pid} className="row-ai ai-table-row" style={{ display: "grid", gridTemplateColumns: "44px 1fr 300px", gap: "0 clamp(12px,2vw,28px)", padding: "8px 16px", borderBottom: i < procs.length - 1 ? "1px solid rgba(0,255,65,.07)" : "none", alignItems: "center", animation: `${glow.anim} ${glow.dur}s ease-in-out infinite`, animationDelay: `${i * 0.3}s`, transition: "padding-left .3s" }}>
                  <span style={{ ...mono, fontSize: 11, color: "rgba(0,255,65,.58)" }}>{p.pid}</span>
                  <div>
                    <span style={{ ...mono, fontSize: 13, color: "#f2f2f2", textShadow: "0 0 6px rgba(0,255,65,.35)" }}>{p.name}</span>
                    <div className="ai-row-desc" style={{ ...mono, fontSize: 12, color: "rgba(0,255,65,.62)", marginTop: 2, lineHeight: 1.4 }}>{p.desc}</div>
                  </div>
...
function PortfolioShot({ lines }: { lines: string[] }) {
  return (
    <div className="portfolio-shot" style={{ position: "relative", aspectRatio: "16/9", overflow: "hidden", background: "#0a0a0a" }}>
      ...
      <div style={{ position: "absolute", inset: 0, top: "14%", padding: "clamp(14px,3vw,22px)", fontFamily: "'JetBrains Mono',monospace", fontSize: "clamp(9px,1vw,11px)", lineHeight: 1.9, whiteSpace: "pre" }}>
        {lines.map((l, i) => {
          const trimmed = l.trimStart();
          const color = trimmed.startsWith("-") ? "#8a6262" : trimmed.startsWith("+") ? "#5ebf8a" : i === 0 ? "#b8b8b8" : "#666";
...
function Services() {
  return (
    <section id="services" style={{ flexShrink: 0, scrollSnapAlign: "start", width: "max(1100px,100vw)", height: "100vh", display: "flex", flexDirection: "column", justifyContent: "center", padding: "84px clamp(20px,4vw,60px) 20px", position: "relative", overflow: "hidden" }}>
      <SectionRain className="services-rain" />
        ...
        <Reveal delay={80}>
          <div className="services-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 1, background: "rgba(0,255,65,.14)", border: "1px solid rgba(0,255,65,.14)" }}>
            {SERVICE_ITEMS.map((svc) => (
              <div key={svc.id} className="card-service" style={{ background: "#000", transition: "background .3s,box-shadow .3s" }}>
                <div className="service-preview" style={{ opacity: .6 }}>{svc.photo ? <Portfolio3DPhoto src={svc.photo} /> : <PortfolioShot lines={svc.code!} />}</div>
                <div className="card-service-body" style={{ padding: "clamp(14px,2.4vh,22px)" }}>
                  <span className="card-service-id" style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 11, color: "#008f11", letterSpacing: ".14em" }}>{svc.id}</span>
                  ...
                  <span className="card-service-rule" style={{ display: "block", width: 28, height: 2, background: "#00ff41", boxShadow: "0 0 6px rgba(0,255,65,.6)", margin: "8px 0 10px" }} />
                  <p className="card-service-desc" style={{ color: "#9a9a9a", fontSize: 12.5, lineHeight: 1.5, fontFamily: "'Montserrat',sans-serif" }}>{svc.desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <MobileScrollHint sectionId="services"/>
    </section>
  );
}
...
const APP_SKINS: Record<"console" | "powershell" | "opencode" | "claude", AppSkin> = {
  console: { label: "~/console", dots: true, border: "rgba(0,255,65,.25)", headerBg: "#0a1a0a", headerBorder: "rgba(0,255,65,.18)", labelColor: "#00a015" },
...
function AppWindow({ skin, innerRef, fill, children }: { skin: AppSkin; innerRef?: React.Ref<HTMLDivElement>; fill?: boolean; children: React.ReactNode }) {
  return (
    <div ref={innerRef} style={{ width: fill ? "100%" : "fit-content", maxWidth: "100%", border: `1px solid ${skin.border}`, boxSizing: "border-box" }}>
      ...
      <div className="app-window-body" style={{ padding: "clamp(14px,2.2vh,22px) clamp(18px,3vw,30px)" }}>{children}</div>
    </div>
  );
}
const PRICE_CTA_STYLE:React.CSSProperties={display:"block",marginTop:"auto",padding:"10px 0",border:"1px solid rgba(0,255,65,.35)",color:"#00ff41",textAlign:"center",fontSize:12,fontFamily:"'JetBrains Mono',monospace",letterSpacing:".08em",textDecoration:"none",transition:"background .2s,border-color .2s"};
...
function PriceFeatureList({items,color}:{items:string[];color:string}){return <ul className="price-features" style={{margin:"10px 0 16px",padding:0,listStyle:"none",display:"flex",flexDirection:"column",gap:5,fontFamily:"'Montserrat',sans-serif"}}>{items.map((f)=><li key={f} style={{display:"flex",gap:8,fontSize:12.5,lineHeight:1.4,color}}><span style={{color:"#00ff41",flexShrink:0}}>›</span>{f}</li>)}</ul>;}
function Price(){return <section id="price" style={{flexShrink:0,scrollSnapAlign:"start",width:"max(1000px,100vw)",height:"100vh",display:"flex",flexDirection:"column",justifyContent:"center",padding:"84px clamp(20px,4vw,60px) 20px",background:"#000",position:"relative",overflow:"hidden"}}><SectionRain/><div style={{position:"relative",zIndex:1}}><Reveal><div className="section-header"><AppWindow skin={APP_SKINS.opencode}><span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:11,color:"#008f11"}}>02 / Стоимость</span><h2 className="section-header-title" style={SECTION_TITLE_STYLE}>Два формата. Цена фиксируется до старта.</h2></AppWindow></div><div className="price-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:1,marginTop:24,background:"rgba(0,255,65,.12)"}}><div className="price-card" style={{background:"#050f05",padding:"clamp(16px,3vh,28px)",fontFamily:"'JetBrains Mono',monospace",display:"flex",flexDirection:"column",transition:"box-shadow .3s"}}><div style={{color:"#f2f2f2",fontSize:11}}>ПАКЕТ «БАЗА»</div><strong className="price-number" style={{display:"block",fontFamily:"'Share Tech Mono',monospace",fontSize:"clamp(24px,5.5vh,38px)",color:"#00ff41",margin:"clamp(8px,1.6vh,14px) 0",whiteSpace:"nowrap"}}>50 000 ₽</strong><p className="price-desc" style={{color:"#9a9a9a",fontFamily:"'Montserrat',sans-serif",fontSize:13.5}}>Сайт-визитка или лендинг.</p><PriceFeatureList items={PRICE_FEATURES.base} color="#9a9a9a"/><a href="#contact" className="btn-price-cta" style={PRICE_CTA_STYLE}>Обсудить →</a></div><div className="price-card" style={{background:"#003b00",padding:"clamp(16px,3vh,28px)",fontFamily:"'JetBrains Mono',monospace",position:"relative",display:"flex",flexDirection:"column",transition:"box-shadow .3s"}}><span style={{position:"absolute",top:0,right:0,background:"#ff0040",color:"#fff",fontSize:9.5,fontWeight:700,letterSpacing:".08em",padding:"5px 10px",textTransform:"uppercase"}}>Популярный выбор</span><div style={{color:"#f2f2f2",fontSize:11}}>ПАКЕТ «ПОЛНЫЙ»</div><strong className="price-number" style={{display:"block",fontFamily:"'Share Tech Mono',monospace",fontSize:"clamp(24px,5.5vh,38px)",color:"#00ff41",margin:"clamp(8px,1.6vh,14px) 0",whiteSpace:"nowrap"}}>100 000 ₽</strong><p className="price-desc" style={{color:"#c8ffe0",fontFamily:"'Montserrat',sans-serif",fontSize:13.5}}>Многостраничный сайт или каталог.</p><PriceFeatureList items={PRICE_FEATURES.full} color="#c8ffe0"/><a href="#contact" className="btn-price-cta" style={{...PRICE_CTA_STYLE,borderColor:"rgba(0,255,65,.55)"}}>Обсудить →</a></div></div></Reveal></div><MobileScrollHint sectionId="price"/></section>;}
...
function Process(){
  ...
  return (
    <section id="process" style={{ flexShrink: 0, scrollSnapAlign: "start", width: "max(1100px,100vw)", height: "100vh", display: "flex", flexDirection: "column", justifyContent: "center", padding: "84px clamp(20px,4vw,60px) 20px", position: "relative", overflow: "hidden" }}>
      <SectionRain />
      <div style={{ position: "relative", zIndex: 1 }}>
        <Reveal>
          <div className="section-header" style={{ borderTop: "1px solid rgba(0,255,65,.18)", paddingTop: 20, marginBottom: 24 }}>
            <span style={{ ...mono, fontSize: 11, color: "#008f11" }}>03 / Процесс</span>
            <h2 className="section-header-title" style={SECTION_TITLE_STYLE}>Четыре шага. Вы видите результат на каждом.</h2>
          </div>
        </Reveal>
        <Reveal delay={80}>
          <div ref={ref} style={{ border: "1px solid rgba(0,255,65,.2)", animation: "borderGlow 4s ease-in-out infinite" }}>
            ...
            {PROCESS_STEPS.map((s, i) => {
              ...
              return (
                <div key={s.n} className="process-row" style={{ padding: "clamp(10px,1.8vh,16px) 16px", borderBottom: i < PROCESS_STEPS.length - 1 ? "1px solid rgba(0,255,65,.07)" : "none", animation: `${glow.anim} ${glow.dur}s ease-in-out infinite`, animationDelay: `${i * 0.3}s` }}>
                  <div style={{ display: "flex", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
                    <span style={{ fontFamily: "'VT323',monospace", fontSize: 16, color: "rgba(0,255,65,.58)", flexShrink: 0 }}>[00:0{i}.{String(i * 23).padStart(3, "0")}]</span>
                    <span style={{ ...mono, fontSize: 13, color: "#f2f2f2", flexShrink: 0 }}>$ {s.name.toLowerCase()}.sh</span>
      ...
      <MobileScrollHint sectionId="process"/>
    </section>
  );
}
...
function Contact(){return <section id="contact" style={{flexShrink:0,scrollSnapAlign:"start",width:"max(1100px,100vw)",height:"100vh",display:"flex",flexDirection:"column",justifyContent:"center",padding:"84px clamp(20px,4vw,60px) 20px",background:"#050f05",borderLeft:"1px solid rgba(0,255,65,.18)",position:"relative",overflow:"hidden"}}><SectionRain/><div className="contact-grid" style={{position:"relative",zIndex:1,display:"grid",gridTemplateColumns:"1.1fr .9fr",gap:44,alignItems:"center"}}><div className="section-header"><span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:11,color:"#008f11"}}>04 / Связаться</span><h2 className="section-header-title" style={SECTION_TITLE_STYLE}>Расскажите,<br/>что нужно<br/>сделать.</h2></div><TerminalBox title="~/contact/form.sh" accent><TerminalContactForm/></TerminalBox></div><MobileScrollHint sectionId="contact"/></section>;}
...
// Mobile-only (hidden via CSS by default, re-enabled in the ≤640px media
// query — see KEYFRAMES). These sections now scroll vertically inside their
// own horizontal slot on mobile; this is the "there's more below" cue,
// fading out once the section is scrolled near its own bottom so it never
// lies about there being more content. Desktop never mounts a listener for
// this — the underlying section has no vertical overflow there.
function MobileScrollHint({ sectionId }: { sectionId: string }) {
  ...
  return (
    <div className="mobile-scroll-hint" style={{ position: "absolute", left: 0, right: 0, bottom: 0, height: 64, alignItems: "flex-end", justifyContent: "center", paddingBottom: 10, background: "linear-gradient(180deg,transparent,rgba(0,0,0,.82) 65%)", zIndex: 3, opacity: show ? 1 : 0, transition: "opacity .3s ease" }}>
      <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 10, letterSpacing: ".18em", textTransform: "uppercase", color: "rgba(0,255,65,.65)", animation: "hudBlink 2.6s steps(1) infinite" }}>▾ scroll</span>
    </div>
  );
}
...
const KEYFRAMES=`a:focus-visible,button:focus-visible{outline:2px solid rgba(0,255,65,.6);outline-offset:2px}.input-terminal:focus-visible{outline:none;box-shadow:0 0 0 1px rgba(0,255,65,.55);background:rgba(0,255,65,.06)}@keyframes slideBar{0%{transform:translateX(-100%)}100%{transform:translateX(100%)}} @keyframes marqAnim{from{transform:translateX(0)}to{transform:translateX(-50%)}} @keyframes neonPulse{0%,100%{text-shadow:0 0 10px rgba(0,255,65,.4),0 0 28px rgba(0,255,65,.18)}50%{text-shadow:0 0 20px rgba(0,255,65,.85),0 0 52px rgba(0,255,65,.4)}} @keyframes hudBlink{0%,93%,100%{opacity:1}94%,96%{opacity:0}95%,97%{opacity:1}98%,99%{opacity:.3}} @keyframes borderGlow{0%,100%{border-color:rgba(0,255,65,.2)}50%{border-color:rgba(0,255,65,.5)}} @keyframes rowGlowDone{0%,100%{background:rgba(0,255,65,.02)}50%{background:rgba(0,255,65,.07)}} @keyframes rowGlowActive{0%,100%{background:rgba(0,255,65,.04)}50%{background:rgba(0,255,65,.14)}} @keyframes rowGlowRunning{0%,100%{background:rgba(0,255,65,.03)}50%{background:rgba(0,255,65,.10)}} @keyframes rowGlowQueued{0%,100%{background:rgba(0,255,65,.015)}50%{background:rgba(0,255,65,.05)}} @keyframes matrixHighlight{0%{text-shadow:0 0 2px rgba(0,255,65,.25)}30%{text-shadow:0 0 16px rgba(0,255,65,1),0 0 34px rgba(0,255,65,.65)}100%{text-shadow:0 0 6px rgba(0,255,65,.35)}} @keyframes rippleOut{0%{transform:translate(-50%,-50%) scale(.5);opacity:1}100%{transform:translate(calc(-50% + var(--tx)),calc(-50% + var(--ty))) scale(1);opacity:0}} @keyframes termCursorBlink{0%,49%{opacity:1}50%,100%{opacity:0}} @keyframes fieldError{0%,100%{transform:translateX(0)}25%{transform:translateX(-6px)}75%{transform:translateX(6px)}} @media (hover:hover) and (pointer:fine){.link-nav:hover{color:#00ff41}.btn-next:hover{background:rgba(0,255,65,.12)}.card-service:hover{background:#0a1a0a;box-shadow:inset 0 0 0 1px rgba(0,255,65,.4),inset 0 0 22px rgba(0,255,65,.07)}.price-card:hover{box-shadow:inset 0 0 0 1px rgba(0,255,65,.4),inset 0 0 22px rgba(0,255,65,.07)}.row-ai:hover{padding-left:22px}.portfolio-shot:hover{background:#111}.btn-price-cta:hover{background:rgba(0,255,65,.12)}.link-footer:hover{opacity:.7}.btn-hero-primary:hover{background:#7dffaa;box-shadow:0 0 24px rgba(0,255,65,.5)}.btn-hero-secondary:hover{border-color:rgba(0,255,65,.8);background:rgba(0,255,65,.08)}} @media (min-width:1024px){.hero-grid{grid-template-columns:3fr 2fr!important}} @media (max-width:640px){.nav-links{display:none!important}.nav-toggle{display:inline-flex!important}.hero-hud{display:none!important}.ai-table-head{display:none!important}.ai-table-row{grid-template-columns:28px 1fr!important}.ai-table-row>*:nth-child(3){grid-column:1/-1!important;margin-top:6px}.section-dots{display:none!important}.decode-divider{display:none!important}.mobile-scroll-hint{display:flex!important}#top{scroll-snap-type:x mandatory!important}#hero,#mission,#services,#price,#ai,#process,#contact{width:100vw!important;overflow-y:auto!important;overflow-x:hidden!important;justify-content:flex-start!important;padding-top:64px!important}.services-grid,.price-grid,.contact-grid{grid-template-columns:1fr!important}.service-preview{display:none!important}.services-rain{opacity:.35!important}.card-service-body{padding:12px 14px!important}.card-service-body h3{margin:6px 0 0!important;font-size:18px!important}.card-service-rule{margin:6px 0 8px!important}.card-service-desc{line-height:1.35!important;font-size:12px!important}.price-card{padding:14px 16px!important}.price-features{gap:3px!important}.price-features li{font-size:11.5px!important;line-height:1.3!important}.process-row{padding:8px 14px!important}.section-header{margin-bottom:12px!important}.section-header-title{font-size:22px!important;line-height:1.15!important;margin-top:6px!important}.app-window-body{padding:10px 14px!important}.ai-row-desc{display:none!important}.ai-table-row{padding:6px 14px!important}.card-service-id{display:none!important}.price-card{padding:10px 14px!important}.price-number{font-size:26px!important;margin:6px 0!important}.price-desc{font-size:12px!important;margin:0 0 4px!important}.btn-price-cta{padding:6px 0!important}.price-features{margin-top:4px!important}@supports (height:100dvh){.app-shell,#top,#hero,#mission,#services,#price,#ai,#process,#contact{height:100dvh!important}}}.mobile-scroll-hint{display:none}@media (prefers-reduced-motion: reduce){*,*::before,*::after{animation-duration:0.01ms!important;animation-iteration-count:1!important;transition-duration:0.01ms!important;scroll-behavior:auto!important}}`;
...
function useMainWheelRedirect() {
  useEffect(() => {
    ...
    function onWheel(e: WheelEvent) {
      ...
      isScrolling = true;
      sections[target].scrollIntoView({ behavior: scrollBehaviorPref(), inline: "start", block: "nearest" });
