      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        <Reveal>
          <div style={{ borderTop: "1px solid rgba(0,255,65,.18)", paddingTop: 20, marginBottom: 40 }}>
            ...
            <h2 style={{ fontFamily: "'Montserrat',sans-serif", fontWeight: 700, fontSize: "clamp(22px,3.6vw,52px)", color: "#f2f2f2", marginTop: 18 }}>Один человек отвечает за весь результат.</h2>
          </div>
        </Reveal>
        <Reveal delay={80}>
            ...
            {SERVICE_ITEMS.map((svc) => (
              <div key={svc.id} className="card-service" style={{ background: "#000", transition: "background .3s" }}>
                ...
                <div style={{ padding: "clamp(24px,4vw,44px)" }}>
                  ...
                  <h3 style={{ fontFamily: "'Montserrat',sans-serif", fontWeight: 600, fontSize: "clamp(18px,2.2vw,26px)", color: "#f2f2f2", margin: "14px 0 10px" }}>{svc.name}</h3>
                  <p style={{ color: "#9a9a9a", fontSize: 14, lineHeight: 1.65, fontFamily: "'Montserrat',sans-serif" }}>{svc.desc}</p>
                </div>
              </div>
            ))}
          </div>
...
function Tools() {
  ...
  useEffect(() => {
    ...
    // listens on the whole section (not just the card strip) so scrolling
    // anywhere over the heading/padding also redirects into the horizontal
    // strip — a listener scoped to the ~260px-tall strip alone left most of
    // the section scrolling the page normally, which read as "still vertical".
    ...
    return () => { el.removeEventListener("scroll", onScroll); window.removeEventListener("resize", onScroll); section.removeEventListener("wheel", onWheel); cancelAnimationFrame(raf); };
  }, []);
  return (
    <section id="tools" ref={sectionRef} style={{ padding: "clamp(80px,12vh,140px) 0", background: "#050f05", position: "relative", overflow: "hidden" }}>
        ...
        <Reveal>
          <div style={{ borderTop: "1px solid rgba(0,255,65,.18)", paddingTop: 20, display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: 12 }}>
            <div>
              ...
              <h2 style={{ fontFamily: "'Montserrat',sans-serif", fontWeight: 700, fontSize: "clamp(22px,3.6vw,44px)", color: "#f2f2f2", marginTop: 18, maxWidth: 560 }}>Открыто на экране, пока я работаю.</h2>
            </div>
...
function PriceFeatureList({items,color}:{items:string[];color:string}){return <ul style={{margin:"18px 0 0",padding:0,listStyle:"none",display:"flex",flexDirection:"column",gap:8,fontFamily:"'Montserrat',sans-serif"}}>{items.map((f)=><li key={f} style={{display:"flex",gap:8,fontSize:13,lineHeight:1.5,color}}><span style={{color:"#00ff41",flexShrink:0}}>›</span>{f}</li>)}</ul>;}
function Price(){return <section id="price" style={{padding:"clamp(80px,12vh,140px) clamp(20px,5vw,90px)",background:"#000",position:"relative",overflow:"hidden"}}><SectionRain/><div style={{position:"relative",zIndex:1}}><Reveal><div style={{borderTop:"1px solid rgba(0,255,65,.18)",paddingTop:20}}><span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:11,color:"#008f11"}}>03 / Стоимость</span><h2 style={{fontFamily:"'Montserrat',sans-serif",fontWeight:700,fontSize:"clamp(22px,3.6vw,52px)",color:"#f2f2f2",marginTop:18}}>Два формата. Цена фиксируется до старта.</h2></div><div className="price-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:1,marginTop:50,background:"rgba(0,255,65,.12)"}}><div style={{background:"#050f05",padding:"clamp(22px,5vw,40px)",fontFamily:"'JetBrains Mono',monospace"}}><div style={{color:"#008f11",fontSize:11}}>ПАКЕТ «БАЗА»</div><strong style={{display:"block",fontSize:"clamp(30px,7vw,52px)",color:"#00ff41",margin:"clamp(14px,3vw,25px) 0",whiteSpace:"nowrap"}}>50 000 ₽</strong><p style={{color:"#9a9a9a",fontFamily:"'Montserrat',sans-serif",fontSize:15}}>Сайт-визитка или лендинг.</p><PriceFeatureList items={PRICE_FEATURES.base} color="#9a9a9a"/><a href="#contact" className="btn-price-cta" style={PRICE_CTA_STYLE}>Обсудить →</a></div><div style={{background:"#003b00",padding:"clamp(22px,5vw,40px)",fontFamily:"'JetBrains Mono',monospace"}}><div style={{color:"#7dffaa",fontSize:11}}>ПАКЕТ «ПОЛНЫЙ»</div><strong style={{display:"block",fontSize:"clamp(30px,7vw,52px)",color:"#00ff41",margin:"clamp(14px,3vw,25px) 0",whiteSpace:"nowrap"}}>100 000 ₽</strong><p style={{color:"#c8ffe0",fontFamily:"'Montserrat',sans-serif",fontSize:15}}>Многостраничный сайт или каталог.</p><PriceFeatureList items={PRICE_FEATURES.full} color="#c8ffe0"/><a href="#contact" className="btn-price-cta" style={{...PRICE_CTA_STYLE,borderColor:"rgba(0,255,65,.55)"}}>Обсудить →</a></div></div></Reveal></div></section>;}
const PROCESS_STEPS = [
...
function Process(){return <section style={{padding:"clamp(80px,12vh,140px) clamp(20px,5vw,90px)",position:"relative",overflow:"hidden"}}><SectionRain/><div style={{position:"relative",zIndex:1}}><Reveal><div style={{borderTop:"1px solid rgba(0,255,65,.18)",paddingTop:20,marginBottom:40}}><span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:11,color:"#008f11"}}>04 / Процесс</span><h2 style={{fontFamily:"'Montserrat',sans-serif",fontWeight:700,fontSize:"clamp(22px,3.6vw,52px)",color:"#f2f2f2",marginTop:18}}>Четыре шага. Вы видите результат на каждом.</h2></div></Reveal><Reveal delay={80}><div className="process-grid" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:1,background:"rgba(0,255,65,.14)",border:"1px solid rgba(0,255,65,.14)"}}>{PROCESS_STEPS.map((s)=><div key={s.n} className="card-service" style={{background:"#000",padding:"clamp(22px,3.4vw,32px)",transition:"background .3s"}}><span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:11,color:"#008f11",letterSpacing:".14em"}}>{s.n}</span><h3 style={{fontFamily:"'Montserrat',sans-serif",fontWeight:600,fontSize:"clamp(16px,1.8vw,20px)",color:"#f2f2f2",margin:"12px 0 8px"}}>{s.name}</h3><p style={{color:"#9a9a9a",fontSize:14,lineHeight:1.6,fontFamily:"'Montserrat',sans-serif"}}>{s.desc}</p></div>)}</div></Reveal></div></section>;}
